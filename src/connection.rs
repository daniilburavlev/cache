...
impl Connection {
    ...
    pub async fn read_frame(&mut self) -> Result<Option<Entity>, CacheError> {
        loop {
    ...
    pub async fn write_frame(&mut self, entity: &Entity) -> io::Result<()> {
        match entity {
            Entity::Array(val) => {
                self.stream.write_u8(b'*').await?;
                self.write_decimal(val.len() as i64).await?;
                for e in &**val {
                    self.write_value(e).await?;
                }
            }
            _ => self.write_value(entity).await?,
        }
        self.stream.flush().await
    }

    async fn write_value(&mut self, entity: &Entity) -> io::Result<()> {
        match entity {
            Entity::Simple(val) => {
                self.stream.write_u8(b'+').await?;
